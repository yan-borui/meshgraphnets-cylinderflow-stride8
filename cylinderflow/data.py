"""Lazy prefix-only HDF5 access and Train-only normalization."""
...
class Dataset:
    """Read only frames 0..64; ``initial`` never loads future field values."""
    ...
    def read(self, index: int, start: int = 0, stop: int = FRAMES) -> dict:
        if not 0 <= start < stop <= FRAMES:
            raise ValueError("field access must stay within stored frames 0..64")
...
def load_statistics(file_name: str | Path, dataset: Dataset) -> dict:
    ...
    if stats.get("protocol") != PROTOCOL_VERSION or stats.get("frame_range") != [0, 64]:
        raise ValueError(
            "normalizer must be generated by prepare from Train frames 0..64"
        )
